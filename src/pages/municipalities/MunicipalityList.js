...
const MunicipalityList = () => {
    ...
    const { loading, total, data } = useFetch('/municipalities', {
        perPage: 10,
        page: 1,
        filter: filter
    })
    ...
    const handleDelete = React.useCallback(async (values) => {
        const { data } = await axios.delete(`/municipalities/${values.id}`);

        if (data) {
            setItems(prevItems => [...prevItems.filter(({ id }) => id != data.id)])
            enqueueSnackbar(
                `¡Ha eliminado el municipio "${data.name}"`, 
                { variant: 'success' }
            );
        }
    }, [])
            ...
            <TableRow hover tabIndex={-1} key={row.name}>
                ...
                <TableCell scope="row" align='right' width='10%'>
                    <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                        <LinkIconButton
                            href={`/municipalities/${row.id}`} 
                            icon={<RemoveRedEyeIcon />}
                        />
                        <LinkIconButton href={`/municipalities/${row.id}/edit`} />
                        <DeleteButton
                            title={`¿Está seguro que desea eliminar el municipio "${row.name}"?`}
    ...
    return (
        <ListContainer title="Municipios">
